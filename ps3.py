...
def get_word_score(word, n):
    ...
    returns: int >= 0
    """
    sum1 = 7 * len(word) - 3 * (n - len(word))
    if sum1 < 1: sum1 = 1
    sum2 = 0
    for letter in word.lower():
        sum2 += SCRABBLE_LETTER_VALUES[letter]
    return sum1 * sum2
...
def is_valid_word(word, hand, word_list):
    ...
    def is_valid_no_wildcards(word, hand):
        nonlocal word_list
        temp_hand = hand.copy()
    ...
    if '*' not in word:
        return is_valid_no_wildcards(word, temp_hand)

    for v in VOWELS:
        temp_hand[v] = temp_hand.get(v, 0) + 1
        if is_valid_no_wildcards(word.replace('*', v), temp_hand):
            return True
        temp_hand[v] -= 1
        if temp_hand[v] <= 0:
            del temp_hand[v]
...
def substitute_hand(hand, letter):
    ...
    temp_hand[rand_letter] = temp_hand[letter]
    del temp_hand[letter]
    return temp_hand
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(total_hands):
        ...
        print('Current hand:', end=' ')
        display_hand(hand)
        choice = input('Would you like to substitute a letter? ').lower()
        if choice == 'yes':
        ...
        print(SEPARATOR)
        if replay_unused:
            choice = input('Would you like to replay the hand? ').lower()
            if choice == 'yes':
                replay_unused = False
                print()
                hand_score = play_hand(hand, word_list)
